...
const filtrarEstudiantesAprobados = (estudiantes) => {
  ...
  estudiantes.forEach((element) => {
    let suma = element.calificaciones.reduce(
...
function esPerfecto(num) {
  ...
  return (
    num ===
    numeros.reduce((acumulado, valorActual) => acumulado + valorActual, 0)
  );
}
...
function armstrong(numero) {
  ...
  let arregloComparado = arregloNumeros.map((element) =>
    Math.pow(element, arregloNumeros.length)
  );

  return (
    numero ===
    arregloComparado.reduce(
      (acumulado, valorActual) => acumulado + valorActual,
      0
    )
  );
...
console.log(
  coincidencia("palabra", "Aqui hay varias palabras: palabra, palabra, palabra")
);

function encontrarDiferencia(array1, array2) {
  let diferentes = [];

  array1.forEach((element) => {
    if (!array2.includes(element)) {
      diferentes.push(element);
    }
  });
  return diferentes;
}

console.log(encontrarDiferencia([1, 2, 3, 4, 5], [1, 2, 3]));
